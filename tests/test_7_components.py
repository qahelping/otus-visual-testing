...
@pytest.mark.only
def test_main_page_elements_screenshot(browser):
    ...
    time.sleep(3)
    browser.find_element(By.CSS_SELECTOR, '[id="header"]').screenshot(master_path)

    browser.get('https://www.wildberries.by/')
    time.sleep(3)
    browser.find_element(By.CSS_SELECTOR, '[id="header"]').screenshot(staging_path)
